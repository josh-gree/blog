    ...
    cells = svg.append("g").attr("id", "cells").selectAll("empty")
        .data(corr)
            ...
            .on("mouseover", (d) ->
              ...
              x = document.getElementById('plot_lab').offsetLeft + x
              y = parseFloat d3.select(this).attr('y')
              y = document.getElementById('plot_lab').offsetTop + y - 50
              d3.select(this).attr("stroke", "black")

              tt = d3.select("#tooltip_la")
              ...
              d3.select("#tooltip_lab").classed("hiddenlab", false)
              )
            .on("mouseout", ->
              d3.select(this).attr("stroke","none")
              d3.select("#tooltip_lab").classed("hidden", true)
              )
            # .on("click", ->
            #   window.alert "Hello")
