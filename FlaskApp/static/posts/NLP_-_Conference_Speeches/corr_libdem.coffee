d3.json "http://josh-gree.me/static/data_libdem.json", (data) ->
    # dimensions of SVG
    sims = data.sims
    labels = data.labels
    ...
    cells = svg.append("g").attr("id", "cells").selectAll("empty")
        .data(corr)
            ...
            .on("mouseover", (d) ->
              ...
              x = document.getElementById('plot_libdem').offsetLeft + x + 150
              y = parseFloat d3.select(this).attr('y')
              y = document.getElementById('plot_libdem').offsetTop + y + 50

              d3.select(this).attr("stroke", "black")

              tt = d3.select("#tooltip_libdem").classed("hidden", false)
            ...
            .on("mouseout", ->
              ...
              d3.select("#tooltip_libdem").classed("hidden", true)
              )
            # .on("click", ->
            #   window.alert "Hello")
